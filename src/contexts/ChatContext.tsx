...
type ChatContextType = {
  ...
  typing: boolean;
  onlineUsers: string[];
  setActiveThread: (t: DMThread | null) => void;
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(async ({ data }) => {
        ...
        // Build sidebar with lastMessage & unreadCount
        const otherUserIds = data.map((t) => (t.user1 === user.id ? t.user2 : t.user1));
          ...
          if (!isChatOpen && Notification.permission ==="granted"){
            (async ()=>{
              ...
              new Notification(senderName,{
                body: msg.content || "You received a message",
              });
            })();  
          }
          // Update sidebarThreads
          setSidebarThreads((prev) =>{
            const exists =prev.find((t)=>t.id === msg.thread_id);
            ...
            return prev.map((t) =>
              t.id === msg.thread_id
                ? {
                    ...t,
                    lastMessage: msg,
                    unreadCount: t.unreadCount! + (msg.receiver_id === user.id ? 1 : 0),
                  }
                : t
            )
        });
          //update message if chat is open
          if(isChatOpen){
